...
| `--post-url` | 無効 | 文章完成時に`POST`する広場サーバのURL（例: `http://127.0.0.1:8000/submit`）。`http://`のみ対応、未指定なら送信しない |

### 広場サーバ連携

`--post-url http://<サーバのIP>:8000/submit` を付けて起動すると、5W1Hの文章が完成するたびに
[../server/](../server/) の広場サーバへ `{"device":"<--nameの値>","sentence":"<完成した文章>"}` をPOSTします。
...
Web Viewerの「文章から画像をつくる」欄では、完成を待たずに任意の文章を手入力して画像生成を開始できます。
