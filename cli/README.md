...
画面とAPIはlocalhostだけで公開され、交換データを外部サーバーへ送信しません。フォントのみGoogle Fontsから取得し、取得できない場合はシステムフォントを使用します。静的ファイルはバイナリに埋め込まれるため、配布時に `web/` ディレクトリをバイナリと一緒に置く必要はありません。ポートを変える場合は、例えば `--web --web-port 9000` と指定します。
...
| `--post-url` | 無効 | Web Viewerから画像生成を手動実行する広場サーバのURL（例: `http://127.0.0.1:8000/submit`）。`http://`のみ対応 |

### 広場サーバ連携

`--post-url http://<サーバのIP>:8000/submit` を付けて起動すると、Web Viewerから
[../server/](../server/) の広場サーバへ `{"device":"<--nameの値>","sentence":"<入力した文章>"}` をPOSTできます。
送信後は広場サーバの生成状況(`queued`/`working`/`done`/`error`)をポーリングし、完成した画像のURLを
Web Viewer（`--web`）の画面にも表示します。送信・ポーリングは交換処理と並行して行われ、
失敗してもBLEの交換自体は継続します（標準エラー出力にログのみ）。TLSは未対応です。

文章が完成すると「文章から画像をつくる」欄へ完成文を自動入力しますが、画像生成は自動では始まりません。
Apple Siliconの画像生成サーバを起動しているMacで「画像を生成（Macのみ）」を押すと生成を開始します。
完成を待たずに任意の文章を手入力することもできます。
送信にはCLI起動時のユーザー名を使い、生成状況と完成画像は同じ `http://127.0.0.1:8787` の画面へ表示されます。
この機能にも `--post-url` の指定が必要です。
