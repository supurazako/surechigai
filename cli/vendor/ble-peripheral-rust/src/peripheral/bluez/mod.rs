...
impl PeripheralImpl for Peripheral {
    ...
    async fn start_advertising(&mut self, name: &str, uuids: &[Uuid]) -> Result<(), Error> {
        ...
        // Keep the GATT application registered across advertising pauses. Dropping
        // it on every role change emits Service Changed notifications to macOS and
        // races CoreBluetooth characteristic discovery.
    ...
    async fn stop_advertising(&mut self) -> Result<(), Error> {
        // ApplicationHandle intentionally remains alive until Peripheral is dropped.
