...
impl PeripheralInternal {
    ...
    pub fn set_characteristics(
        ...
        // didModifyServices can clear the service map while an older
        // didDiscoverCharacteristics callback is still queued. Treat that callback
        // as stale instead of panicking the CoreBluetooth worker thread.
    ...
    fn check_discovered(&mut self) {
        ...
        if !self.services.values().any(|service| !service.discovered) {
            let Some(future) = self.services_discovered_future_state.take() else {
                warn!("Ignoring a late duplicate CoreBluetooth discovery callback");
                return;
            };
            let services = self
                .services
            ...
            future.lock().unwrap().set_reply(reply);
        }
    }
...
impl CoreBluetoothInternal {
            ...
            && let Some(future) = p.services_discovered_future_state.take()
        {
            future
                ...
                .set_reply(CoreBluetoothReply::Err(format!(
                    "Stale characteristic discovery for unknown service {service_uuid}"
                )));
        }
    }
    ...
    fn write_value(
        ...
        let Some(peripheral) = self.peripherals.get_mut(&peripheral_uuid) else {
            Self::complete_missing(fut, "Peripheral");
            return;
        };
        let Some(service) = peripheral.services.get_mut(&service_uuid) else {
            Self::complete_missing(fut, "Service");
            return;
        };
        let Some(characteristic) = service.characteristics.get_mut(&characteristic_uuid) else {
            Self::complete_missing(fut, "Characteristic");
            return;
        };
        {
    ...
    fn write_descriptor_value(
        ...
        let Some(descriptor) = characteristic.descriptors.get_mut(&descriptor_uuid) else {
            Self::complete_missing(fut, "Descriptor");
            return;
        };
        {
            ...
            delegate_msg = self.delegate_receiver.select_next_some() => {
                match delegate_msg {
                    ...
                    CentralDelegateEvent::CharacteristicSubscribed{
                        ...
                        characteristic_uuid,
                        error,
                     } => self.on_characteristic_subscribed(peripheral_uuid, service_uuid, characteristic_uuid, error),
                    CentralDelegateEvent::CharacteristicUnsubscribed{
                        peripheral_uuid,
                        service_uuid,
                        characteristic_uuid,
                        error,
                     } => self.on_characteristic_unsubscribed(peripheral_uuid, service_uuid,characteristic_uuid, error),
